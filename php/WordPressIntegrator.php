...
class WordPressIntegrator {
    ...
    public function testConnection() {
        try {
            $response = $this->makeRequest('GET', '/wp-json/wp/v2/users/me');
            return [
                'success' => true,
                'user' => $response
            ];
    ...
    public function getSiteInfo() {
        try {
            $response = $this->makeRequest('GET', '/wp-json');
            return [
                'success' => true,
                'info' => $response
            ];
...
if (!$connectionTest['success']) {
    die('WordPress connection failed: ' . $connectionTest['error']);
}

// Assuming $content comes from AIContentGenerator
$content = [
    'sitemap' => [...],
    'pages' => [...],
    'blog_articles' => [...]
...
$results = $wpIntegrator->createWebsite($content);

if ($results['success']) {
    echo "Website created successfully!\n";
    echo "Pages created: " . count($results['pages']) . "\n";
    echo "Posts created: " . count($results['posts']) . "\n";
} else {
